...
class FileControllerTest {
    ...
    void whenFileFoundThenReturnContent() {
        ...
        when(fileService.getFileById(1L)).thenReturn(Optional.of(fileDto));

        ResponseEntity<?> response = fileController.getById(1L);
    ...
    void whenFileNotFoundThenReturnNotFound() {
        when(fileService.getFileById(99L)).thenReturn(Optional.empty());

        ResponseEntity<?> response = fileController.getById(99L);
